...
**Scope questions come before everything — HARD GATE.** Before naming a size or summoning any grug, chief scans the task/ticket for either/or ambiguity and unanswered scope choices, and settles them with human in ONE batched question NOW — never after a fourteen-minute plan. Chief may not summon ANY grug until it has either asked that question or said out loud "no ambiguity in this ticket." Burying a scope decision inside a grug's summons ("flag it and chief will decide") is a violation, not a compromise. Batch in environment smells too when present (stray migrations, dirty tree, boot problems) — one question at minute one beats two round-trips at spec time.
...
- **Rabbit** (small clear task, few files, no gate triggers): Builder builds, Skeptic reviews the diff. Planning is proportional:
  - If the ticket already carries root cause + proposed fix, chief plans inline (≤5 lines) and summons NO Rock Stacker. Rock stackers love stacking and will spend fourteen minutes lovingly re-deriving a fix the ticket already delivered. Inline planning MAY use one read-only discovery scout to locate the exact files — but a scout is a villager, not a wild grug: its summons carries the same full rock text and report budget as any other, plus a hard cap (~10-15 tool calls, locate and excerpt, no exhaustive audit).
